...
import { updateTouchInputRect, updateTouchInput } from './touchpad';
...
class ResponsiveGamepadService {
  ...
  enable(keyMap) {
    ...
    if (typeof window !== "undefined") {
      ...
      const updateKeyboardHandler = updateKeyboard.bind(this);
  ...
  disable() {
    ...
    this.keyMapKeys.forEach(keyMapKey => {
      this.keyMap[keyMapKey].TOUCHPAD.forEach((input) => {
        input.UNLISTEN();
      });
    });
    Object.keys(this.analogMaps).forEach(analogMapsKey => {
      this.analogMaps[analogMapsKey].TOUCH_INPUT.UNLISTEN();
    });
  ...
  addTouchInput(element, inputType) {
    ...
    let touchInputId = createTouchInput(element, inputType, updateTouchInputRect.bind(this), updateTouchInput.bind(this), this.keyMap, this.analogMaps, additionalArguments);
  ...
  removeTouchInput(touchInputId) {
    ...
    this.keyMapKeys.forEach(keyMapKey => {
      this.keyMap[keyMapKey].TOUCHPAD.forEach((input, index) => {
        if (input.ID === touchInputId) {
          input.UNLISTEN();
          this.keyMap[keyMapKey].TOUCHPAD.splice(index, 1);
          foundId = true;
        }
      });
    });
    ...
    Object.keys(this.analogMaps).forEach(analogMapsKey => {
      if (analogMapsKey === touchInputId) {
        this.analogMaps[analogMapsKey].TOUCH_INPUT.UNLISTEN();
        delete this.analogMaps[analogMapsKey];
        foundId = true;
      }
    });
  ...
  getState() {
    ...
    Object.keys(this.analogMaps).forEach(analogMapsKey => {
      Object.keys(this.analogMaps[analogMapsKey]).forEach(analogMapKey => {
        ...
        const stateValue = this.analogMaps[analogMapsKey][analogMapKey];
        if (stateValue) {
